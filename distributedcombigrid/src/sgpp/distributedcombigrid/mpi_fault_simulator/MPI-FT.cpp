...
int simft::Sim_FT_Check_dead_processes(simft::Sim_FT_MPI_Comm f_comm, simft::NextOp option) {
  ...
  if (f_comm->comm_rank == f_comm->Root_Rank) {
    ...
    if (option == simft::NextOp::Comm_free) {
      addReserveSize += boost::numeric_cast<int>(f_comm->NBC_Vector_Send.size());  // NBC_Vector_Send will be attached, reserve its size
    }
    ...
    //
    Send_Vector.push_back(boost::numeric_cast<int>(f_comm->Dead_Processes_Root.size()));
...
void simft::Sim_FT_Perform_background_operations(simft::Sim_FT_MPI_Comm f_comm,
  ...
  if (f_comm->Root_Rank == f_comm->comm_rank) {
    ...
    if (PFlag == 1) {
      ...
      MPI_Isend(&NBCVec[0], boost::numeric_cast<int>(NBCVec.size()), MPI_INT, f_comm->Root_Rank, SIM_FT_NBC_RESPONSE_TAG, f_comm->c_comm_copy, &NBC_Request);
...
void simft::Sim_FT_Perform_background_operations_dead(simft::Sim_FT_MPI_Comm f_comm) {
  if (f_comm->Root_Rank == f_comm->comm_rank) {
    ...
    if ((int)simft::Sim_FT_MPI_COMM_WORLD->Dead_Processes_Root.size() ==
      ...
      //
      Send_Vector.push_back(boost::numeric_cast<int>(simft::Sim_FT_MPI_COMM_WORLD->Dead_Processes_Root.size()));
      ...
      simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &(simft::Sim_FT_MPI_COMM_WORLD->Dead_Buf), boost::numeric_cast<int>(Send_Vector.size()), MPI_INT, SIM_FT_DEAD_TAG, simft::Sim_FT_MPI_COMM_WORLD->c_comm_copy_coll,
  ...
  if (PFlag == 1) {
    ...
    NBCVec = simft::NBC_to_Vector(f_comm);
    // send serialized list to root
    MPI_Isend(&NBCVec[0], boost::numeric_cast<int>(NBCVec.size()), MPI_INT, f_comm->Root_Rank, SIM_FT_NBC_RESPONSE_TAG,
...
void simft::Sim_FT_NBC_bcast_root(simft::Sim_FT_MPI_Comm f_comm) {
  if (f_comm->CurrentMinNBC_Modified) {
    if (f_comm->LastNBC_bcast + SIM_FT_WAIT_NBC_BCAST < MPI_Wtime()) {
      ...
      simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &Buffer_Vector, boost::numeric_cast<int>(Send_Vector.size()), MPI_INT,
...
void simft::Sim_FT_Perform_nb_operations(std::vector<simft::Sim_FT_Istats> *NB_ops_to_perform,
  ...
  if (f_comm->Recent_Icollectives.size() <
    ...
    for (int i = boost::numeric_cast<int>(f_comm->Recent_Icollectives.size()); i < boost::numeric_cast<int>(NB_ops_to_perform->size()); i++) {
...
void simft::Sim_FT_Complete_nb_operations(
   ...
   */
  for (unsigned int i = boost::numeric_cast<unsigned int>(f_comm->Recent_Icollectives.size()); i < (*Deserialized_NBC_Vector).size();
       i++) {
   ...
   */
  MPI_Waitall(boost::numeric_cast<int>(Requests_To_Finish.size()), &Requests_To_Finish.front(), MPI_STATUSES_IGNORE);
...
void simft::Sim_FT_kill_me() {
  ...
  for (auto Actives : simft::Sim_FT_Current_Active_Communicators) {
    MPI_Request Send_Request;
    int NBC = boost::numeric_cast<int>(Actives->Recent_Icollectives.size());
    ...
    Actives->Send_Vector[1] = boost::numeric_cast<int>(Actives->Recent_Icollectives.size());
      ...
      if (Actives->Root_Rank != Actives->comm_rank) {
        ...
        if (simft::Sim_FT_Custom_Ireduce(Actives, true)) {
          ...
          if (Actives->Last_Send_Vector[3] == 3) {
            ...
            reserveAddSize += (boost::numeric_cast<int>(Actives->NBC_Vector_Send.size()));
          }
          ...
          Send_Vector.push_back(boost::numeric_cast<int>(Actives->Dead_Processes_Root.size()));
          ...
          simft::Sim_FT_Custom_Ibcast_Root(&Send_Vector, &(Actives->Dead_Buf), boost::numeric_cast<int>(Send_Vector.size()),
...
void simft::Sim_FT_Custom_Dead_bcast(std::vector<int> *buf, simft::Sim_FT_MPI_Comm f_comm,
  ...
  if (f_comm->comm_rank == f_comm->Root_Rank) {
    ...
    simft::Sim_FT_Custom_Ibcast_Root(buf, &(f_comm->Dead_Buf), boost::numeric_cast<int>(buf->size()), MPI_INT, BcastTag,
...
int simft::Sim_FT_Custom_Reduce(int Send_Vector[], simft::Sim_FT_MPI_Comm f_comm,
  ...
  Send_Vector[0] = 0;
  Send_Vector[1] = boost::numeric_cast<int>(f_comm->Recent_Icollectives.size());
...
bool simft::Sim_FT_Custom_Ireduce(simft::Sim_FT_MPI_Comm f_comm,
  ...
  if (f_comm->Received_Cnt < f_comm->Bcast_Successors.size()) {
    ...
    if (f_comm->comm_rank != f_comm->Root_Rank) {  // if root, process has to access
      ...
      MPI_Isend(&f_comm->Last_Send_Vector[0], boost::numeric_cast<int>(f_comm->Last_Send_Vector.size()), MPI_INT,
    ...
    f_comm->Send_Vector[1] = boost::numeric_cast<int>(f_comm->Recent_Icollectives.size());

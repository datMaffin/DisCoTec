...
int MPI_Comm_shrink(simft::Sim_FT_MPI_Comm f_comm, simft::Sim_FT_MPI_Comm* newcomm) {
  simft::Sim_FT_Check_dead_processes(f_comm, simft::NextOp::Default);  // in case of revoked comm
                                                                       // make sure all processes
                                                                       // switched sync tags
...
int MPI_Comm_agree(simft::Sim_FT_MPI_Comm f_comm, int* flag) {
  // important: if comm is revoked, an initial Default sync ensures, all processes have switched to
  // the revoked sync tags
  ...
  if (f_comm->Last_Send_Vector[0] ==
      4) {  // This value will be the same at all processes. 4 => false; 5 => true
    *flag = 0;
  } else {
    *flag = 1;
  }

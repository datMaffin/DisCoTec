...
void ProcessGroupWorker::processDuration(const Task& t, const Stats::Event e, size_t numProcs) { 
  MASTER_EXCLUSIVE_SECTION {
    ...
    durationInformation info = {t.getID(), Stats::getEventDurationInUsec(e), t.getCurrentTime(), t.getCurrentTimestep(), theMPISystem()->getWorldRank(), boost::numeric_cast<unsigned int>(numProcs)};
...
void ProcessGroupWorker::combineUniform() {
  ...
  int numGrids = boost::numeric_cast<int>(combiParameters_.getNumGrids());
...
void ProcessGroupWorker::parallelEvalUniform() {
  ...
  assert(combiParametersSet_);
  int numGrids = boost::numeric_cast<int>(combiParameters_

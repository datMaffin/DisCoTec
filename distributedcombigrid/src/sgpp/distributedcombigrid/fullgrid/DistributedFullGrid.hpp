...
class DistributedFullGrid {
 public:
  ...
  void writePlotFile(const char* filename) const {
    ...
    // write subarray
    MPI_File_write_all(fh, getData(), boost::numeric_cast<int>(getNrLocalElements()), getMPIDatatype(), MPI_STATUS_IGNORE);
 ...
 private:
  ...
  void InitMPI(MPI_Comm comm) {
    ...
    if (status == MPI_CART) {
      ...
      MPI_Cart_get(comm, boost::numeric_cast<int>(maxdims), &cartdims[0], &periods[0], &coords[0]);
